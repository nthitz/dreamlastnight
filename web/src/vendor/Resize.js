...
var sourceOfWorker = "../resizeWorker.js"
function Resize(widthOriginal, heightOriginal, targetWidth, targetHeight, blendAlpha, interpolationPass, useWebWorker, resizeCallback, returnImage) {
...
Resize.prototype.postprocess = function(data) {
	if(! this.returnImage) {
		return data
	}
	...
	newCanvas.width = this.targetWidth
	newCanvas.height = this.targetHeight
	var ctxt = newCanvas.getContext('2d')
	var imageBuffer = ctxt.createImageData(this.targetWidth, this.targetHeight)
	for (var x = 0; x < data.length; ++x) {
		imageBuffer.data[x] = data[x] & 0xFF;
	}
	console.log(imageBuffer.data.length)
	ctxt.putImageData(imageBuffer, 0, 0)
...
Resize.prototype.configureWorker = function () {
	try {
		var parentObj = this;
		this.worker = new Worker(sourceOfWorker);
...
Resize.prototype.resize = function (buffer) {
	if (this.useWebWorker) {
		...
		this.resizeCallback(this.postprocess(this.resizeHeight(this.resizeWidth(buffer))));
	}
}
Resize.prototype.resizeImage = function(img) {
